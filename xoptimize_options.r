...
risk_aversion <- 4.0
budget <- 1000.0
optimization_objectives <- c("sharpe", "sharpe_adj", "mean_variance")
...
for (optimization_objective in optimization_objectives) {
  ...
  if (optimization_objective == "mean_variance") {
    ...
    stop("optimization_objective must be 'mean_variance', 'sharpe', 'adjusted_sharpe', or 'sharpe_adj'")
  }
  ...
  summary_rows <- rbind(
    ...
    data.frame(
      ...
      terminal_adj_sharpe = as.numeric(portfolio_adj_sharpe),
      terminal_skew = as.numeric(portfolio_payoff_stats["skew"]),
      terminal_ex.kurt = as.numeric(portfolio_payoff_stats["ex.kurt"]),
