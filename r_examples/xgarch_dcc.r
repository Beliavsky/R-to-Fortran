...
return_stats <- function(x) {
  # Return basic sample moments and extrema for one return series.
  c(
    mean = mean(x),
    sd = sd(x),
    min = min(x),
...
soft_ab <- function(a_raw, b_raw, persistence_max = 0.999) {
  # Map unconstrained persistence parameters to stable alpha and beta values.
  ea <- exp(a_raw)
  eb <- exp(b_raw)
  denom <- 1 + ea + eb

  c(
    alpha_star = persistence_max * ea / denom,
    beta = persistence_max * eb / denom
  )
}

make_ab_start <- function(alpha_star0, beta0, persistence_max = 0.999) {
  # Convert target alpha and beta starts to unconstrained optimizer parameters.
...
unpack_garch_par <- function(par) {
  # Transform unconstrained optimizer parameters into GARCH model parameters.
  mu <- par[1]
  omega <- exp(par[2])
...
make_garch_start_par <- function(x) {
  # Build a feasible starting parameter vector for a GARCH-t fit.
  mu0 <- mean(x)
  v0 <- var(x)
...
unpack_nagarch_par <- function(par) {
  # Transform unconstrained optimizer parameters into NAGARCH model parameters.
  mu <- par[1]
  omega <- exp(par[2])
  theta <- par[3]
...
make_nagarch_start_par <- function(x) {
  # Build a feasible starting parameter vector for a NAGARCH-t fit.
  mu0 <- mean(x)
  v0 <- var(x)
...
garch_filter <- function(x, p) {
  # Run the GARCH variance recursion and return residual diagnostics.
  mu <- p$mu
  omega <- p$omega
  alpha <- p$alpha
  beta <- p$beta
...
nagarch_filter <- function(x, p) {
  # Run the NAGARCH variance recursion and return residual diagnostics.
  mu <- p$mu
  omega <- p$omega
  alpha <- p$alpha
  theta <- p$theta
...
garch_negloglik <- function(par, x) {
  # Compute the negative Student-t log likelihood for a GARCH parameter vector.
...
nagarch_negloglik <- function(par, x) {
  # Compute the negative Student-t log likelihood for a NAGARCH parameter vector.
...
fit_garch_t <- function(x, maxit = 2000) {
  # Estimate one univariate GARCH-t model and package fitted diagnostics.
...
fit_nagarch_t <- function(x, maxit = 2000) {
  # Estimate one univariate NAGARCH-t model and package fitted diagnostics.
...
dcc_unpack <- function(par) {
  # Transform unconstrained DCC parameters into stable correlation dynamics.
  ea <- exp(par[1])
  eb <- exp(par[2])
  denom <- 1 + ea + eb
  ...
  c(a = a, b = b, persistence = a + b)
}

dcc_start_par <- function(a0 = 0.03, b0 = 0.94) {
  # Convert target DCC starts to unconstrained optimizer parameters.
...
dcc_negloglik <- function(par, Z) {
  # Compute the DCC Gaussian correlation negative log likelihood.
  ab <- dcc_unpack(par)
  ...
  for (t in 2:n) {
    ...
    # chol_R is upper triangular and t(chol_R) %*% chol_R = Rt.
...
dcc_filter <- function(Z, par) {
  # Run the DCC recursion and return time-varying correlation matrices.
  ab <- dcc_unpack(par)
...
fit_dcc <- function(Z, maxit = 2000) {
  # Estimate DCC correlation dynamics for standardized residuals.
  start <- dcc_start_par()
...
make_ccc_covariances <- function(hmat, R) {
  # Combine conditional variances with a constant correlation matrix.
  n <- nrow(hmat)
  k <- ncol(hmat)
...
make_dcc_covariances <- function(hmat, R_array) {
  # Combine conditional variances with time-varying DCC correlations.
  n <- nrow(hmat)
  k <- ncol(hmat)
...
# -----------------------------
# Fit multivariate models: CCC and optional DCC
# -----------------------------
...
if (nasset < 2) {
  ...
  if (fit.dcc) {
    ...
    cat("\nLast DCC conditional correlation matrix R_t\n")
    ...
    cat("\nLast DCC conditional covariance matrix H_t\n")
    print(round(H_dcc_last, 6))

    dcc_last_vol <- sqrt(diag(H_dcc_last))
    names(dcc_last_vol) <- asset_names

    cat("\nLast conditional standard deviations from DCC model\n")
    print(round(dcc_last_vol, 6))

    cat("\nDCC covariance array dimensions:\n")
